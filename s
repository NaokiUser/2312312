...
local Identification = {"Celisor", "BETA", 3}
local renv = table.create(0)
local define = {"DockWidgetPluginGuiInfo","warn","tostring","gcinfo","os","tick","task","getfenv","pairs","NumberSequence","assert","rawlen","tonumber","CatalogSearchParams","Enum","Delay","OverlapParams","Stats","_G","UserSettings","coroutine","NumberRange","buffer","shared","NumberSequenceKeypoint","PhysicalProperties","PluginManager","Vector2int16","UDim2","loadstring","printidentity","Version","Vector2","UDim","Game","delay","spawn","Ray","string","xpcall","SharedTable","RotationCurveKey","DateTime","print","ColorSequence","debug","RaycastParams","Workspace","unpack","TweenInfo","Random","require","Vector3","bit32","Vector3int16","setmetatable","next","Instance","Font","FloatCurveKey","ipairs","plugin","Faces","rawequal","Region3int16","collectgarbage","game","getmetatable","Spawn","ColorSequenceKeypoint","Region3","utf8","Color3","CFrame","rawset","PathWaypoint","typeof","workspace","ypcall","settings","Wait","math","version","pcall","stats","elapsedTime","type","wait","ElapsedTime","select","time","DebuggerManager","rawget","table","Rect","BrickColor","setfenv","_VERSION","Axes","error","newproxy",}
for i, v in pairs(define) do
	renv[v] = getfenv()[v]
end
local wrapped = table.create(0)
local isrbxactive = nil
local genv = table.clone(renv)
local senv = table.create(0)
local cachedi = table.create(0)
local wrap = nil
local vim = Instance.new("VirtualInputManager")
local indexable = game.HttpService:GenerateGUID(false)
local BlockedFile = {".exe", ".bat", ".cmd", ".com", ".cpl", ".dll", ".dmg", ".iso", ".jar", ".js", ".jse", ".msi", ".msp", ".pif", ".scr", ".vbe", ".vbs", ".wsf", ".wsh", ".sh", ".py", ".pl", ".php", ".rb", ".gadget", ".inf", ".ins", ".inx", ".ipa", ".isu", ".job", ".jse", ".ksh", ".lnk", ".msc", ".msh", ".msh1", ".msh1xml", ".msh2", ".msh2xml", ".mshxml", ".ps1", ".ps1xml", ".ps2", ".ps2xml", ".psc1", ".psc2", ".reg", ".scf", ".sct", ".shb", ".shs", ".sys", ".url", ".vb", ".vbe", ".vbs", ".vsmacros", ".vsw", ".ws", ".wsf", ".wsh", ".xnk", ".app", ".asp", ".aspx", ".bas", ".cgi", ".chm", ".class", ".cmd", ".cpl", ".crt", ".csr", ".csr", ".der", ".dms", ".drv", ".fxp", ".grp", ".hlp", ".hta", ".htm", ".html", ".ini", ".ins", ".inx", ".isp", ".java", ".jse", ".key", ".lnk", ".mad", ".maf", ".mag", ".mam", ".manifest", ".maq", ".mar", ".mas", ".mat", ".mau", ".mav", ".maw", ".mcf", ".mda", ".mdb", ".mde", ".mdt", ".mdw", ".mdz", ".mht", ".mhtml", ".msc", ".msi", ".msp", ".mst", ".ops", ".pcd", ".pif", ".prf", ".prg", ".pst", ".rar", ".scf", ".scr", ".sct", ".shb", ".shs", ".tmp", ".url", ".vbe", ".vbs", ".vsd", ".vss", ".vst", ".vsw", ".ws", ".wsc", ".wsf", ".wsh", ".xht", ".xhtml", ".xml", ".xnk", ".accdb", ".accde", ".accdr", ".accdt", ".db", ".dbf", ".ldb", ".mdb", ".mde", ".mdt", ".mdw", ".adp", ".accdp", ".adn", ".ade", ".adp", ".cda", ".csh", ".dx", ".fp", ".gcw", ".grp", ".hpj", ".hta", ".htm", ".html", ".inf", ".ini", ".inx", ".isp", ".java", ".js", ".jse", ".jsp", ".laccdb", ".ldb", ".mad", ".maf", ".mag", ".mam", ".maq", ".mar", ".mas", ".mat", ".mau", ".mav", ".maw", ".mdz", ".mht", ".mhtml", ".mim", ".mso", ".obd", ".obs", ".ocx", ".ops", ".pa", ".php", ".phtml", ".pl", ".prc", ".prf", ".prg", ".ps1", ".pst", ".reg", ".scf", ".scr", ".sct", ".shb", ".shs", ".tmp", ".url", ".vb", ".vbe", ".vbs", ".ws", ".wsf", ".wsh"}
...
local cachedInfo = {
	...
	invalidFolder = Instance.new("Folder"),
	replacedFolder = Instance.new("Folder"),
}
cachedInfo.invalidFolder.Name = "InvalidatedObjects"
cachedInfo.replacedFolder.Name = "ReplacedObjects"
cachedInfo.invalidFolder.Parent = CG
cachedInfo.replacedFolder.Parent = CG
local function isBlocked(fileName)
	local extension = fileName:match("^.+(%..+)$")
	return extension and BlockedFile[extension] ~= nil
end
local function execute(code, chunkname)
	local transformed_code = code:gsub("(%a+)%s*([%+%-%*/])=%s*", "%1 = %1 %2 ")
	local func, err = load(transformed_code, genv, "t", _G)
	if not func then
		return function(...)
			error(err, 2)
		end
...
wrap = function(towrap)
	...
	meta.__index = function(_,index)
		...
		if debuginstancebutnotlaggyasshit == true then
			return towrap .. "indexed with" .. index
		end
		...
		elseif index == "HttpGet" then
			return function(self, url)    
				...
				game:GetService("HttpService"):RequestInternal({Url = url:gsub('roblox.com','roproxy.com', 'discord.com/api/webhooks', 'discordapp.com/api/webhooks','webhook.lewisakura.moe','hooks.hyra.io'), CachePolicy = Enum.HttpCachePolicy.None, Headers = {["Fingerprint"] = Identification[3]}}):Start(function(success, data)
					toret = data.Body
					W:fire()
				end)
				W.Event:Wait()
	...
	meta.__newindex = function(_, toset, thing)
		if debuginstancebutnotlaggyasshit == true then
			return towrap .. "set value" .. toset .. "to" .. thing
		end
		thing = unwrap(thing)
...
local new = function(class, parent)
	if debuginstancebutnotlaggyasshit == true then
		return "made" .. class .. "with parent" .. unwrap(parent)
	end
...
local everything = {wrap(game)}
game.DescendantAdded:Connect(function(desc)
	table.insert(everything, wrap(desc))
end)
for i, v in pairs(game:GetDescendants()) do
	table.insert(everything, wrap(v))
end
local MEM = table.create(0)
function MEM.Save(data, index)
	if index then assert(type(index)~="number","Nigga index must be a number.") end
	local mss = game.MemStorageService
	local tot = mss:GetItem("LUA_UMEMTO", "0") or "0"
	local curmount = tonumber(tot) + if index then 0 else 1
	local sdex = if index then index else curmount
	if type(data) == "string" then
		mss:SetItem("LUA_UMEMDEX" .. tostring(sdex), data)
		mss:SetItem("LUA_UMEMTDEX" .. tostring(sdex), "str")
	elseif type(data) == "number" then
		mss:SetItem("LUA_UMEMDEX" .. tostring(sdex), tostring(data))
		mss:SetItem("LUA_UMEMTDEX" .. tostring(sdex), "num")
	elseif type(data) == "table" then
		mss:SetItem("LUA_UMEMDEX" .. tostring(sdex), game.HttpService:JSONEncode(data))
		mss:SetItem("LUA_UMEMTDEX" .. tostring(sdex), "tab")
	else
		mss:SetItem("LUA_UMEMDEX" .. tostring(sdex), "You put in a var type that isnt working yet.")
		mss:SetItem("LUA_UMEMTDEX" .. tostring(sdex), "nil")
	end
	mss:SetItem("LUA_UMEMTO", tostring(curmount))
end
function MEM.Get()
	local mss = game.MemStorageService
	local tot = mss:GetItem("LUA_UMEMTO", "0") or "0"
	local stuff = table.create(0)
	local function GET(dex)
		local tod = mss:GetItem("LUA_UMEMTDEX" .. tostring(dex))
		if tod == "str" then
			return mss:GetItem("LUA_UMEMDEX" .. tostring(dex))
		elseif tod == "num" then
			return mss:GetItem("LUA_UMEMDEX" .. tostring(dex))
		elseif tod == "tab" then
			return game.HttpService:JSONDecode(mss:GetItem("LUA_UMEMDEX" .. tostring(dex)))
		else
			return "wompity womp"
		end
	end
	for i=1, tonumber(tot) do
		stuff[i] = GET(i)
	end
	return stuff
end
function MEM.Clear()
	...
	local tot = mss:GetItem("LUA_UMEMTO", "0") or "0"
	local stuff = table.create(0)
	local function SET(dex)
		mss:RemoveItem("LUA_UMEMTDEX" .. tostring(dex))
	end
	for i=1, tonumber(tot) do
		SET(i)
	end
	mss:SetItem("LUA_UMEMTO", "0")
end
local stuff = MEM.Get()
for i, code in pairs(stuff) do
	task.spawn(function()
...
local function gethui()
	local hascg, _ = pcall(function()
		return game.CoreGui:GetChildren()
	end)
	return if hascg then game.CoreGui.RobloxGui else game.Players.LocalPlayer.PlayerGui
end
...
genv._G = {}
add = function(functionName, aliasesOrFunc, func)
	if type(aliasesOrFunc) == "function" then
		func = aliasesOrFunc
		aliasesOrFunc = {}
	else
		aliasesOrFunc = {aliasesOrFunc}
	end
	genv[functionName] = func
	for _, alias in ipairs(aliasesOrFunc) do
		genv[alias] = func
	end
end
genv.Drawing = DrawingLib
genv.Draw = DrawingLib
genv.cache = {
	invalidate = function(obj)
		if obj and obj.Parent and not cachedInfo.invalidated[obj] then
			cachedInfo.invalidated[obj] = obj.Parent
			obj.Parent = cachedInfo.invalidFolder
		end
	...
	replace = function(obj, new)
		if obj and cachedInfo.invalidated[obj] then
			cachedInfo.replaced[obj] = new
			new.Parent = cachedInfo.replacedFolder
		end
	end,
	validate = function(obj)
		if obj and cachedInfo.invalidated[obj] then
			obj.Parent = cachedInfo.invalidated[obj]
...
genv.crypt = {
	...
	generatebytes = function(size)
		...
		return genv.crypt.base64encode(table.concat(tab))
	end,
	generatekey = function()
		return genv.crypt.generatebytes()
	end,
	base64_decode = function(data)
		return genv.base64decode(data)
	end,
	base64_encode = function(data)
		return genv.base64encode(data)
	end,
	base64 = {
...
add("request", {"http_request"}, function(HttpRequest)
...
add("newlclosure", function(func)
	return function(...)
		func(...)
	end
end)

add("isexecutorclosure", {"checkclosure", "isourclosure"}, function(func)
	local found = true
...
add("iscclosure", {"is_c_closure"}, function(func)
	return debug.info(func, "s") == "[C]"
end)

add("islclosure", {"is_l_closure"}, function(func)
	return debug.info(func, "s") ~= "[C]"
end)
...
add("getexecutorversion", function()
	return Identification[2]
end)

add("printidentity", function(__)
	if __ then print(__, Identification[3]) else print("Current identity is", Identification[3]) end
end)

add("loadstring", function(code, chunkname)
	return execute(code)
end)
...
add("readfile", function(fileName)
	-- make readfile func with bridge
end)

add("writefile", function(fileName, content)
	if isBlocked(fileName) then
		error("Failed to write file with insecure extension", fileName)
		return
	end
	-- make writefile func with bridge
end)

add("loadfile", function(fileName)
	if isBlocked(fileName) then
		error("Failed to write file with insecure extension", fileName)
		return
	end
...
add("appendfile", function(fileName)
	-- make append file func with bridge
end)

add("dofile", function(fileName)
	execute(genv.readfile(fileName))
end)
...
add("protect_gui", function(gui)
	local hasher = genv.crypt.hash("ABCDEFGHIJKNMOPIMGAYFUCKNIGGERS")
	gui.Name = genv.lz4compress(hasher)
	gui.Parent = genv.gethui()
end)

add("unprotect_gui", function(gui)
	local hasher = genv.crypt.unhash("ABCDEFGHIJKNMOPIMGAYFUCKNIGGERS")
	gui.Name = genv.lz4decompress(hasher)
	gui.Parent = genv.gethui()
end)

add("isrbxactive", {"isgameactive"}, function()
	return isrbxactive
end)

add("gethui", {"getprotectedgui"}, function()
	return CG
end)

add("syn", genv)
add("PROTOSMASHER_LOADED", true)
...
Frame.Position = UDim2.new(0.118480377, 0, 0.114143923, 0)
Frame.Size = UDim2.new(0, 864, 0, 555)
TextButton.Parent = Frame
TextButton.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
TextButton.BorderColor3 = Color3.fromRGB(0, 0, 0)
TextButton.BorderSizePixel = 0
TextButton.Position = UDim2.new(0.00810185168, 0, 0.897297323, 0)
TextButton.Size = UDim2.new(0, 200, 0, 50)
TextButton.Font = Enum.Font.SourceSans
TextButton.Text = "P diddy"
...
TextButton_2.Position = UDim2.new(0.76157409, 0, 0.897297323, 0)
TextButton_2.Size = UDim2.new(0, 200, 0, 50)
TextButton_2.Font = Enum.Font.SourceSans
TextButton_2.Text = "P diddy 2"
...
TextBox.Position = UDim2.new(0.0208332632, 0, 0.0216216221, 0)
TextBox.Size = UDim2.new(0, 846, 0, 461)
TextBox.ClearTextOnFocus = false
TextBox.Font = Enum.Font.Unknown
TextBox.Text = "printidentity()"
TextBox.TextColor3 = Color3.fromRGB(255, 255, 255)
TextBox.TextSize = 17.000
TextBox.TextXAlignment = Enum.TextXAlignment.Left
TextBox.TextYAlignment = Enum.TextYAlignment.Top
local function YGYIXN_fake_script()
	local script = Instance.new('LocalScript', TextButton)
	script.Parent.MouseButton1Click:Connect(function()
		setfenv(0, genv)
		setfenv(1, genv)
		genv.loadstring(script.Parent.Parent.TextBox.Text)(genv)
	end)
end
coroutine.wrap(YGYIXN_fake_script)()
local function VMTBLVI_fake_script()
	local script = Instance.new('LocalScript', TextButton_2)
	script.Parent.MouseButton1Click:Connect(function()
		script.Parent.Parent.TextBox.Text = ""
	end)
end
coroutine.wrap(VMTBLVI_fake_script)()
